...
import { ISiteScrapperAdapter } from '@domain/interfaces/site-scrapper/SiteScrapperAdapter';
...
export class SiteScrapperFactory<B, P> implements ISiteScrapperFactory<B, P> {
  constructor (
    @inject("SiteScrapperAdapter") private browserAdapter: ISiteScrapperAdapter<B, P>,
  ...
  async create <B,P>(
    ...
    if (site.name !== scrapperName) {
      throw new Error("Site name and site scraping data site name are incompatible")
    }
    ...
    return new ParticularSiteScrapper(scrapingSiteEntity, this.browserAdapter, this.streamsAdapter)
  }
}
