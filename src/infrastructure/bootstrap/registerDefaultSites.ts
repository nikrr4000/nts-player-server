import { SiteRepository } from "@domain/repositories/SiteRepository.js";

export const registerDefaultSites = async () => {
    const {defaultSites} = await import("@infrastructure/config/defaultSites.js")
    const {container} = await import("@shared/container.js");
    const {logger} = await import("@infrastructure/logging/logger.js")
    const {SiteFactory} = await import("@domain/entities/Site.js")
